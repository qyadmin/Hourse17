...
namespace BestHTTP.SocketIO
{
    public sealed class SocketManager : IHeartbeat, IManager
    {
        ...
        bool IManager.OnTransportConnected(ITransport transport)
        {
            ...
            if (Poller != null && Poller.State == TransportStates.Open &&
                (Transport == null || Transport.State == TransportStates.Open))
            {
        ...
        void IManager.OnTransportError(ITransport trans, string err)
        {
                ...
                trans.State == TransportStates.Opening)
            {
                ...
                // WebSocket transport failed to upgrade, but we can fall back to poller:
                if (trans == Transport &&
                    Poller != null &&
                    Poller.State == TransportStates.Open)
                {
                    Transport = null;
                ...
                else // Can't fall back, have to try again
                    (this as IManager).TryToReconnect();
            }
            else
            {
        ...
        private ITransport SelectTransport()
        {
            ...
            // Check our main transport. Hopefully a WebSocketTransport.
            if (Transport != null && Transport != Poller)
            {
                if (Transport.State == TransportStates.Open)
                {
                    if (Poller != null && Poller.State == TransportStates.Open && Poller.IsRequestInProgress)
                        return null;

                    return Transport;
                }

                if (Transport.State == TransportStates.Opening)
                    return null;
            }

            // If the main transport isn't open, we will fallback to the polling transport.
            if (Poller != null && Poller.State == TransportStates.Open && !Poller.IsRequestInProgress)
                return Poller;
